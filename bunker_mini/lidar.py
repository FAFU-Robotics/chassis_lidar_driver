...
# 障碍扇区图只关心接近水平的通道（垂直角越小越接近水平面）。
# 通道 1~16 覆盖 -0.07°~13.98°，足够地面机器人近场避障。
...
class ObstacleSectors:
    ...
    def add(self, azimuth_deg: float, distance_m: float) -> None:
        ...
        idx = int(azimuth_deg) % self._sector_count
        cur = self._dist[idx]
...
def transform_point_cloud(
    ...
    *,
    ...
    外参均为 0 时原样返回（零开销路径）。
    """
    if not points or (pitch_deg == 0.0 and lidar_height_m == 0.0):
        return points
...
class AiryLidar:
    ...
    NO_DATA_STALE_S: float = 3.0  # 超过该秒数未收到帧视为雷达离线
