...
class ApproachConfig:
    ...
    approach_distance_m: float = 0.5      # 机械臂工作距离（车头到目标）
...
class ApproachController:
    ...
    def update(self, est: Optional[TargetEstimate]) -> tuple[float, float]:
        ...
        if dist > cfg.approach_distance_m + cfg.distance_tolerance_m:
            self._state = ApproachState.APPROACHING
            self._stabilize_start = None
            v = cfg.max_linear_m_s * min(
                1.0, (dist - cfg.approach_distance_m) * cfg.approach_gain)
        ...
        if now - self._stabilize_start >= cfg.stabilize_s:
            ...
            logger.info("Approach READY at %.2f m", dist)
            if self._on_ready:
                try:
                    self._on_ready(est)
                except Exception:
